...
ffmpeg -i "${INPUT}" \
  ...
  -map "[v2]" -map 0:a -vf "scale=1280:720" -c:v libx264 -profile:v main -level 3.1 \
  ...
  -map "[v3]" -map 0:a -vf "scale=1920:1080" -c:v libx264 -profile:v high -level 4.0 \
...
echo ""
echo "En tu componente usa: /videos/${NAME}/master.m3u8"
